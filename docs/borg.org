...
Borg assimilates packages into the ~~/.config/emacs~ [fn:1] repository
as Git submodules.  An assimilated package is called a drone and a
borg-based ~~/.config/emacs~ repository is called a collective.

It is possible to clone a package repository without assimilating it.
A cloned package is called a clone.

To learn more about this project, also read the blog post [fn:2] in
which it was announced.

[fn:1] Or ~~/.emacs.d~ of course, if you prefer that or have to use the
old location because you still have to support older Emacs releases.
[fn:2] https://emacsair.me/2016/05/17/assimilate-emacs-packages-as-git-submodules.
...
adding a simple ~early-init.el~ file containing:
...
Changing the values of ~load-prefer-newer~ and ~package-enable-at-startup~
as shown here is optional but strongly recommended.  ~load-prefer-newer~
should be enabled before loading ~borg~.
...
#+begin_src sh
  ...
  git submodule add --name llama git@github.com:tarsius/llama.git lib/llama
...
Magit using Borg, as described in [[*Assimilation]].  You should also
...
The early init file, ~early-init.el~ (see [[info:emacs#Early Init File]]),
should contain a call to ~borg-initialize~.
...
Borg automatically reports how long it takes to load ~borg.el~ itself,
~early-init.el,~ and other config files that it loads directly.  It
cannot do the same for ~init.el~, so you should explicitly call the
following function from that file, and possibly from other home-made
config files.

- Function: borg-report-load-duration &optional file format-string ::

  This function reports how long it takes to load the file currently
  being loaded.  It should be called very early in the file, ideally
  before doing anything else.  This prints two messages; one right
  when the function is called and another once the file has been
  loaded.

  If optional FILE is non-~nil~, this function instead reports how long
  it takes to load that file (this assumes that it is currently being
  loaded).  Optional FORMAT-STRING, if non-~nil~, is passed to ~message~.
  It should should end with "..." and contain zero or one ~%s~, which
  stands for the file name.

- Function: borg-report-init-duration ::

  This function adds functions to ~after-init-hook~, which, once that
  hook is nearly done running, will report how long it took to run
  that hook and also how long Emacs initialization took.  Calling
  this function does nothing if the hook already ran.

* Assimilation

A third-party package is assimilated by adding it as a submodule and,
if necessary, by configuring it in ~~/.config/emacs/init.el~.  Built-in
...
using ~borg-assimilate~, or remove the clone using ~borg-remove~.
...
- Command: borg-activate clone ::
  ...
  This command activates the clone named CLONE by adding the
...
status buffer of the ~~/.config/emacs~ repository and press ~f m~ to fetch
...
the status buffer of ~~/.config/emacs~ press ~q~.
...
record the changes in the ~~/.config/emacs~ repository.  To do so
...
~~/.config/emacs~ repository.  Additionally, and ideally when you first
...
have to be in ~~/.config/emacs~ in a shell.  They are implemented in
~borg.mk~, which is part of the ~borg~ package.

To show the commands that are run, use ~V=1 make ...~.  Otherwise only
...
- Command: clean-force ::
  ...
  This target removes all byte-code files using ~find~.  The ~clean~
...
- Command: build ::
  ...
  the user init files, like ~init-build~ does.
...
- Command: native ::
  ...
  it also byte-compiles the user init files, like ~init-build~ does.
...
variable ~submodule.DRONE.build-step~ is set.  The assumption is that
...
- Command: build/DRONE ::
  ...
  ~lib/DRONE~ is an alias for this target; or rather ~DIR/DRONE~, where
...
- Command: init-build ::
  ...
  If you publish your ~~/.config/emacs~ repository but would like to
  keep some settings private, then you can do so by putting them in a
  file ~~/.config/emacs/LOGIN.el~.  The downside of this approach is
...
- Command: bootstrap ::
  ...
  submodule init~, ~make clone-modules~, ~make checkout-modules~ and ~make
  build~.
  ...
  If you have run ~make clone-modules~ and/or ~make checkout-modules~
  manually, then make sure to finish by running ~make build~ again too.

- Command: clone-modules ::

  This target attempts to clone all modules that were initialized by
  ~git submodule init~ and which the user did not explicitly deactivate
  afterwards, either by de-initializing it again (~git submodule deinit
  lib/DRONE~) or by setting ~submodule.DRONE.active~ to ~false~.
  ...
  Alternatively drones can be cloned individually, using
  ~lib/borg/borg.sh clone DRONE~.

- Command: checkout-modules ::

  This target attempts to checkout commits recorded for all modules,
  which were successfully cloned by ~make clone-modules~, while also
  ...
  Alternatively drones can be checked out individually, using
...
If you want to change the value of the following variable, you have to
do so before requiring ~borg~, else it does not have the desired effect.

- Variable: borg-debug-user-init-files ::

  This variable controls whether the effect of the command-line
  argument ~--debug-init~ extends to user init files.  If non-~nil~ and
  ~--debug-init~ is used, then ~debug-on-error~ is automatically enable
  and ~use-package-verbose~ is set to ~debug~.  Finally these variables
  are reset to their default values using ~after-init-hook~.
...
file ~~/.config/emacs/.gitmodules~.  The variables ~borg.pushDefault~ and
...
- Variable: submodule.DRONE.build-step COMMAND ::
  ...
  a lisp expression.  In that case the variable ~borg-clone~ holds
...
- Variable: borg.extra-build-step COMMAND ::
  ...
  build-steps for each DRONE (or after ~submodule.DRONE.build-step,~ if
  that specified).  It has to be set in ~borg-gitmodules-file~ and can
...
- Function: borg-get clone variable &optional all ::
  ...
  ~submodule.CLONE.VARIABLE~ defined in ~~/.config/emacs/.gitmodules~.
...
- Function: borg-get-all clone variable ::
  ...
  ~submodule.CLONE.VARIABLE~ defined in ~~/.config/emacs/.gitmodules~ as a
  list.
...
#+begin_quote
Copyright (C) 2016-{{{year}}} Jonas Bernoulli <emacs.borg@jonas.bernoulli.dev>
